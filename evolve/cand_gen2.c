/* cand_gen2.c — best-of-breed FastLanes scan (evolve gen 2).
 *
 * Synthesis of four gen-1 winners. Dispatch the fastest proven path per width:
 *   - b==8  : byte-aligned cmpeq_epi8 + (&1) + sad_epu8  (from cand_bytealigned, ~13.6 v/ns)
 *   - b==16 : halfword-aligned cmpeq_epi16 + u16 accumulators (from cand_bytealigned, ~9.4)
 *   - b==32 : simple 2-bank uint32 streaming cmpeq (DRAM-bound, ~3.9 beats SIMD funnel 3.2)
 *   - all other widths 1-7, 9-15, 17-31 : cand_vecpos per-bit-width AVX2 specialization
 *     (funnel-shift with compile-time-immediate off/wp), the best overall path (geomean 7.48)
 *
 * No new algorithms — every path is a graft of already-correct gen-1 code.
 */
#include <stdint.h>
#include <stddef.h>
#include <immintrin.h>
...
/* ============================================================================
 * vecpos per-bit-width specializations (base path, widths 1-31 except 8/16).
 * ==========================================================================*/
...
/* ============================================================================
 * b == 8 : byte-aligned (graft from cand_bytealigned). 8 words/block, each word
 * = 4 byte values. cmpeq_epi8 -> & 1 -> sad_epu8 to sum without u8 overflow.
 * ==========================================================================*/
...
/* ============================================================================
 * b == 16 : halfword-aligned (graft from cand_bytealigned). 16 words/block, each
 * word = 2 halfword values. cmpeq_epi16 -> sub into u16 accumulators (<=1024 fits).
 * ==========================================================================*/
...
/* ============================================================================
 * b == 32 : simple multi-bank uint32 streaming compare (DRAM-bound). Each word
 * is one value; compare all 1024 words/block to target. Two+ independent
 * accumulator banks keep the OoO core busy while waiting on memory.
 * ==========================================================================*/
static uint64_t scan_b32(const uint32_t *planar, size_t blocks, uint32_t target) {
    ...
    for (size_t blk = 0; blk < blocks; blk++) {
        ...
        __m256i acc0 = _mm256_setzero_si256();       /* two independent banks */
        __m256i acc1 = _mm256_setzero_si256();
        for (uint32_t w = 0; w < words; w += 32) {
            acc0 = _mm256_sub_epi32(acc0, _mm256_cmpeq_epi32(_mm256_loadu_si256((const __m256i *)(base + w +  0)), tgt));
            acc1 = _mm256_sub_epi32(acc1, _mm256_cmpeq_epi32(_mm256_loadu_si256((const __m256i *)(base + w +  8)), tgt));
            acc0 = _mm256_sub_epi32(acc0, _mm256_cmpeq_epi32(_mm256_loadu_si256((const __m256i *)(base + w + 16)), tgt));
            acc1 = _mm256_sub_epi32(acc1, _mm256_cmpeq_epi32(_mm256_loadu_si256((const __m256i *)(base + w + 24)), tgt));
        }
        count += hsum_epu32(_mm256_add_epi32(acc0, acc1));
    }
    return count;
}
...
uint64_t fl_scan_candidate(const uint32_t *planar, size_t n, unsigned b, uint32_t target) {
    ...
    if (b == 0) return 0;
    uint32_t m = mask_b(b);
    __m256i vmask   = _mm256_set1_epi32((int)m);
    ...
    switch (b) {
        ...
        case 8:  return scan_b8    (planar, blocks, target & m);
        ...
        case 16: return scan_b16   (planar, blocks, target & m);
